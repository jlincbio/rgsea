...
# rgsea.R, a new R implementation of Broad Inst. GSEA
# first cleanup: 10/25/19

# Original copyright notice agreement attached here:

# The Broad Institute SOFTWARE COPYRIGHT NOTICE AGREEMENT
# This software and its documentation are copyright 2003 by the Broad Institute / Massachusetts Institute of Technology. All rights are reserved. This software is supplied without any warranty or guaranteed support whatsoever. Neither the Broad Institute nor MIT can be responsible for its use, misuse, or functionality.
...
# This function ranks the genes according to the signal to noise ratio for the actual phenotype and also random permutations and bootstrap subsamples of both the observed and random phenotypes. It uses matrix operations to implement the signal to noise calculation in stages and achieves fast execution speed. It supports two types of permutations: random (unbalanced) and balanced. 
# It also supports subsampling and bootstrap by using masking and multiple-count variables. When "fraction" is set to 1 (default) the there is no subsampling or boostrapping and the matrix of observed signal to noise ratios will have the same value for all permutations. This is wasteful but allows to support all the multiple options with the same code. Notice that the second matrix for the null distribution will still have the values for the random permutations (null distribution). This mode (fraction = 1.0) is the defaults, the recommended one and the one used in the examples.
# It is also the one that has be tested more thoroughly. The resampling and boostrapping options are intersting to obtain smooth estimates of the observed distribution but its is left for the expert user who may want to perform some sanity checks before trusting the code.
...
# obs.order.matrix: Matrix with the orderings that will sort the columns of the s2n.matrix in decreasing s2n order
	...
	return(list(
		s2n.matrix = s2n.matrix, obs.s2n.matrix = obs.s2n.matrix,
...
# Computes the weighted GSEA score of gene.set in gene.list. The weighted score type is the exponent of the correlation weight: 0 (unweighted = Kolmogorov-Smirnov), 1 (weighted), and 2 (over-weighted). When the score type is 1 or 2 it is necessary to input the correlation vector with the values in the same order as in the gene list.
...
# Plots a heatmap "pinkogram" of a gene expression matrix including phenotype vector and gene, sample and phenotype labels
	...
	plot(1:n.cols, 1:(n.rows + 1), t(heatm), col = mycol, axes = FALSE, main = main, xlab = xlab, ylab = ylab)
...
# Reads a gene expression dataset in RES format and converts it into an R data frame
	header.cont <- readLines(file, n = 1)
	temp <- unlist(strsplit(header.cont, "\t"))
	colst <- length(temp)
	header.labels <- temp[seq(3, colst, 2)]
	ds <- read.table(file, header = FALSE, row.names = 2, sep = "\t", skip = 3, blank.lines.skip = T, comment.char = "", as.is = TRUE)
	colst <- length(ds[1,])
	cols <- (colst - 1) / 2
	rows <- length(ds[,1])
...
# Reads a gene expression dataset in GCT format and converts it into an R data frame
	y <- read.table(file, header = TRUE, sep = "\t", skip = 2, row.names = 1, blank.lines.skip = TRUE, comment.char = "", as.is = TRUE)
	col.retain <- grep("description", tolower(colnames(y)), invert = TRUE)
	return(y[,col.retain])
}
...
mat.normalize <- function(V, margin = 1, na.rm = TRUE) {
	# matrix normalization, margin = 1 (row) and 2 for column
	m <- apply(V, margin, mean, na.rm = na.rm)
	s <- apply(V, margin, sd, na.rm = na.rm)
	n <- dim(V)[margin]
	for (i in 1:n) {
		if (s[i] == 0) {
			if (margin > 1) {
...
GSEA <- function(input.ds, input.cls, gene.ann = "", gs.db, gs.ann = "", 
	output.directory = getwd(), doc.string = "GSEA.analysis",
	reshuffling.type = "sample.labels", nperm = 1000, weighted.score.type = 1,
	nom.p.val.threshold = -1, fwer.p.val.threshold = -1, 
	fdr.q.val.threshold = 0.25, topgs = 10, adjust.FDR.q.val = F, 
	gs.size.threshold.min = 25, gs.size.threshold.max = 500, reverse.sign = F,
	preproc.type = 0, random.seed = NULL, perm.type = 0, fraction = 1.0, 
	replace = F, save.intermediate.results = FALSE, use.fast.enrichment.routine = TRUE) {
...
log.filename <- sprintf("%s/%s_%s", output.directory, doc.string, "params.txt")
log.messages <- c(
	sprintf("Time: %s", as.POSIXlt(Sys.time(),"GMT")),
	paste("output.directory =", output.directory),
	paste("doc.string =", doc.string),
	paste("reshuffling.type =", reshuffling.type), 
	paste("nperm =", nperm), 
	paste("weighted.score.type =", weighted.score.type), 
	paste("nom.p.val.threshold =", nom.p.val.threshold), 
	paste("fwer.p.val.threshold =", fwer.p.val.threshold), 
	paste("fdr.q.val.threshold =", fdr.q.val.threshold),
	paste("topgs =", topgs),
	paste("adjust.FDR.q.val =", adjust.FDR.q.val), 
	paste("gs.size.threshold.min =", gs.size.threshold.min), 
	paste("gs.size.threshold.max =", gs.size.threshold.max), 
	paste("reverse.sign =", reverse.sign),
	paste("preproc.type =", preproc.type), 
	paste("perm.type =", perm.type), 
	paste("fraction =", fraction), 
	paste("replace =", replace)
)

if (!is.data.frame(input.ds)) {
	log.messages <- c(log.messages, paste("input.ds =", input.ds))
}
if (!is.list(input.cls)) {
	log.messages <- c(log.messages, paste("input.cls =", input.cls)) 
}
if (!is.data.frame(gene.ann)) {
	log.messages <- c(log.messages, paste("gene.ann =", gene.ann)) 
}
if (grepl(".gmt$", gs.db)) {
	log.messages <- c(log.messages, paste("gs.db =", gs.db)) 
}
if (!is.data.frame(gs.ann)) {
	log.messages <- c(log.messages, paste("gs.ann =", gs.ann))
}
if (!is.null(random.seed)) {
	set.seed(seed = random.seed, kind = NULL)
	log.messages <- c(log.messages, paste("random.seed =", random.seed))
}
cat(log.messages, file = log.filename, sep = "\n")

# set memory.limit(6000000000) if (.Platform$OS.type == "windows") ?
# <------------- here 10/25
...
if (is.data.frame(input.ds)) {
	...
	if (regexpr(pattern = ".gct", input.ds) == -1) {
		dataset <- read.res(input.ds)
	} else {
		dataset <- read.gct(input.ds)
	}
}
...
# Read gene and gene set annotations if gene annotation file was provided
 ...
 if (is.data.frame(gene.ann)) {
  ...
  temp <- read.table(gene.ann, header = T, sep = ",", comment.char = "", as.is = T)
  a.size <- length(temp[,1])
  print(c("Number of gene annotation file entries:", a.size)) 
  accs <- as.character(temp[,1])
  locs <- match(gene.labels, accs)
 ...
 if (is.data.frame(gs.ann)) {
  ...
  temp <- read.table(gs.ann, header = T, sep = "\t", comment.char = "", as.is = T)
  a.size <- length(temp[,1])
  print(c("Number of gene set annotation file entries:", a.size)) 
  accs <- as.character(temp[,1])
  locs <- match(gs.names, accs)
 ...
 for (i in 1:Ng) {
   ...
   if (Obs.ES[i] >=0) {
	...
	if (save.intermediate.results) {
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "phi.txt")
		write.table(phi, file = filename, quote = F, col.names = F, row.names = F, sep = "\t")
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "obs.phi.txt")
		write.table(obs.phi, file = filename, quote = F, col.names = F, row.names = F, sep = "\t")
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "phi.norm.txt")
		write.table(phi.norm, file = filename, quote = F, col.names = F, row.names = F, sep = "\t")
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "obs.phi.norm.txt")
		write.table(obs.phi.norm, file = filename, quote = F, col.names = F, row.names = F, sep = "\t")
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "Obs.ES.txt")
		write.table(Obs.ES, file = filename, quote = F, col.names = F, row.names = F, sep = "\t")
		filename <- sprintf("%s/%s.%s", output.directory, doc.string, "Obs.ES.norm.txt")
...
# Plots a heatmap plot of a consensus matrix
  ...
  par(mar = c(5, 15, 15, 5))
  plot(1:cols, 1:cols, t(B), col = col.map, axes = FALSE, main = main, sub = sub, xlab = xlab, ylab = ylab)
...
# Plots a heatmap of a matrix
  ...
  par(mar = c(7, 15, 5, 5))
  plot(1:n.cols, 1:n.rows, t(heatm), col = color.map, axes = FALSE, main = main, sub = sub, xlab = xlab, ylab = ylab)
